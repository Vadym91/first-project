...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="38" height="33" alt="handle" src="'+o.imagePath+'handlev.png" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			$('#drag'+randID).width(2).height(imgHeight).css({'background':o.dividerColor, 'background-color': 'red', 'position':'absolute','left':'18px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			$('#handle'+randID).css({'z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'-18px'})
			...
			function drag()
			{
				...
				$('div:eq(2)', obj).width( parseInt( $(this).css('left') ) + 20 );
			}
